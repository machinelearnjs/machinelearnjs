...
import {
  lasso_l2_snap,
  ridge_l1_snap
} from './__snapshots__/manual_cd_regressor.snap';
...
describe('linear_model:Lasso', () => {
  ...
  it('should throw an error if degree or l1 is not provided', () => {
    expect(() => new Lasso({ l1: null })).toThrow(
      'Lasso cannot be initiated with null l1'
    );
    expect(() => new Lasso({ degree: null })).toThrow(
      'Lasso cannot be initiated with null degree'
    );
  });
});
